...
interest <- function(start, destination, column) {
  return (flights %>% filter(origin == start, destination == dest) %>% select_(column))
}

# Retireve the air_time column for flights from JFK to SEA
jkfsea <- interest('JFK', 'SEA', 'air_time')
